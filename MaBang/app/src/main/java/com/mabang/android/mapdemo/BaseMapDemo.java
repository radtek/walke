package com.mabang.android.mapdemo;
...
//        LatLng center = new LatLng(23.117, 113.276); // 广州(纬度，经度)
//        float zoom = 11.0f; // 默认 11级
//        Intent intent = getIntent();
//        if (null != intent) {
//            mEnableCustomStyle = intent.getBooleanExtra("customStyle", true);
//            center = new LatLng(intent.getDoubleExtra("y", 23.117),
//                    intent.getDoubleExtra("x", 113.276));
...
//        MapStatus build = mapBuilder.target(center).zoom(zoom).build();//walke+
//        baiduMapOptions.mapStatus(build);//walke+
//        mMapView = new MapView(this, baiduMapOptions);
        setLocation( new LatLng(23.117, 113.276),11.0f);
        initView(this);
    ...
    private void setLocation(LatLng latLng,float scaleType) {
        MapStatus.Builder mapBuilder = new MapStatus.Builder();
//        LatLng center = new LatLng(39.915071, 116.403907); // 默认 天安门
        float zoom = 11.0f; // 默认 11级
        BaiduMapOptions baiduMapOptions = new BaiduMapOptions();
        MapStatus build = mapBuilder.target(latLng).zoom(zoom).build();//walke+
        baiduMapOptions.mapStatus(build);//walke+
        mMapView = new MapView(this, baiduMapOptions);
    }
    ...
    private void setMapCustomFile(Context context, String PATH) {
        ...
        MapView.setCustomMapStylePath(moduleName + "/" + PATH);//自定义个性化地图
